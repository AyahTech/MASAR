...
function AgentVoicePill({
  ...
  // Disabled (TTS off) OR no enabled provider ⇒ muted pill. Still clickable:
  // nothing configured → open Settings → TTS; TTS merely off → flip it on. The
  // pill is never a dead end (#665 — don't silently hide the control either).
  if (disabled) {
      ...
      <div
        ...
        title={
          availableProviders.length === 0
            ? t('agentBar.voiceSetupHint')
            : t('agentBar.voiceEnableHint')
        }
        onClick={(e) => {
          e.stopPropagation();
          handleDisabledClick();
        ...
        className="flex items-center gap-1.5 h-6 w-[100px] rounded-full bg-muted/40 hover:bg-muted/70 px-2.5 text-[11px] text-muted-foreground/50 hover:text-muted-foreground shrink-0 cursor-pointer transition-colors"
      >
...
function TeacherVoicePill({
  ...
  // Disabled (TTS off) OR no enabled provider ⇒ muted pill. Still clickable:
  // nothing configured → open Settings → TTS; TTS merely off → flip it on.
  if (disabled) {
    const handleDisabledClick = () => {
      if (availableProviders.length === 0) {
        onOpenSettings?.();
...
export function AgentBar({ onOpenSettings }: { onOpenSettings?: () => void }) {
  const { t } = useI18n();
