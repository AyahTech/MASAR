...
export function TTSSettings({ selectedProviderId }: TTSSettingsProps) {
  ...
  return (
    <div className={cn('space-y-6', isVoxCPM ? 'max-w-5xl' : 'max-w-3xl')}>
      ...
      {selectedProviderId === 'elevenlabs-tts' && <ElevenLabsVoiceManager />}
...
interface ElevenLabsAccountVoice {
  id: string;
  name: string;
  language?: string;
  gender?: string;
...
/**
 * Fetches the user's ElevenLabs voice library (including dashboard-added
 * Arabic voices) into ttsProvidersConfig['elevenlabs-tts'].customVoices so
 * they appear in the per-agent voice picker. Multilingual models speak both
 * English and Arabic with the same voice timbre.
 */
function ElevenLabsVoiceManager() {
  const { t } = useI18n();
  const providerConfig = useSettingsStore((state) => state.ttsProvidersConfig['elevenlabs-tts']);
  const setTTSProviderConfig = useSettingsStore((state) => state.setTTSProviderConfig);
  const [fetchStatus, setFetchStatus] = useState<'idle' | 'fetching' | 'error'>('idle');
  const [fetchMessage, setFetchMessage] = useState('');

  const voices = (providerConfig?.customVoices as ElevenLabsAccountVoice[] | undefined) || [];
  ...
  const handleFetch = async () => {
    ...
    try {
      ...
      const data = (await res.json()) as {
        success?: boolean;
        data?: { voices: ElevenLabsAccountVoice[] };
        message?: string;
        detail?: string;
      };
      if (!res.ok || !data.success || !data.data?.voices) {
        setFetchStatus('error');
        setFetchMessage(data.detail || data.message || t('settings.elevenlabsFetchFailed'));
        return;
      }
      setTTSProviderConfig('elevenlabs-tts', { customVoices: data.data.voices });
      setFetchStatus('idle');
      setFetchMessage(t('settings.elevenlabsFetchSuccess', { count: data.data.voices.length }));
    } catch (error) {
      setFetchStatus('error');
      setFetchMessage(error instanceof Error ? error.message : t('settings.elevenlabsFetchFailed'));
    }
  };

  const removeVoice = (voiceId: string) => {
    setTTSProviderConfig('elevenlabs-tts', {
      customVoices: voices.filter((v) => v.id !== voiceId),
    });
  };
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-2">
        <Label className="text-sm">{t('settings.elevenlabsAccountVoices')}</Label>
        <Button
          variant="outline"
          size="sm"
          onClick={handleFetch}
          disabled={fetchStatus === 'fetching' || !providerConfig?.apiKey}
        ...
        >
          ...
          {fetchStatus === 'fetching'
            ? t('settings.elevenlabsFetching')
            : t('settings.elevenlabsFetchVoices')}
        </Button>
      </div>
      ...
      {fetchMessage && (
        <p
          className={cn(
            'text-xs',
            fetchStatus === 'error' ? 'text-destructive' : 'text-muted-foreground',
          )}
        >
          {fetchMessage}
        </p>
      ...
      {voices.length > 0 ? (
        <div className="rounded-lg border border-border/60 overflow-hidden">
          <div className="grid grid-cols-[1fr_auto_auto_28px] gap-0 bg-muted/40 px-3 py-1.5 border-b border-border/40">
            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider">
              {t('settings.voiceNamePlaceholder')}
            </span>
            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider w-14 text-center">
              Lang
            </span>
            <span className="text-[11px] font-medium text-muted-foreground uppercase tracking-wider w-16 text-center">
              {t('settings.elevenlabsVoiceGender')}
            </span>
            <span />
          </div>
          {voices.map((voice, index) => (
            <div
              key={voice.id}
              className={cn(
                'grid grid-cols-[1fr_auto_auto_28px] gap-0 items-center px-3 py-2 group hover:bg-muted/20 transition-colors',
                index > 0 && 'border-t border-border/30',
              )}
            >
              <span className="text-sm text-foreground/80 truncate pr-3" title={voice.description}>
                {voice.name}
              </span>
              {voice.language ? (
                <span className="text-[10px] px-1.5 py-0 h-4 leading-4 rounded bg-muted text-muted-foreground w-14 text-center">
                  {voice.language}
                </span>
              ) : (
                <span className="w-14" />
              ...
              {voice.gender ? (
                <span className="text-[10px] px-1.5 py-0 h-4 leading-4 rounded bg-muted text-muted-foreground w-16 text-center truncate">
                  {voice.gender}
                </span>
              ) : (
                <span className="w-16" />
              ...
              <Button
                variant="ghost"
                size="icon"
                title={t('settings.elevenlabsRemoveVoice')}
                onClick={() => removeVoice(voice.id)}
                className="h-6 w-7 opacity-0 group-hover:opacity-100 transition-opacity text-muted-foreground hover:text-destructive"
              >
                <XCircle className="h-3.5 w-3.5" />
              </Button>
            </div>
        ...
        <p className="text-sm text-muted-foreground/50 italic">
          {t('settings.elevenlabsNoVoicesHint')}
        </p>
      )}
    </div>
  );
