...
import {
  awaitSceneSpeech,
  prefetchSceneSpeech,
  sceneNeedsSpeechAudio,
} from '@/lib/audio/playback-tts-prefetch';
...
export const PlaybackChromeRoot = forwardRef<PlaybackChromeRootHandle, PlaybackChromeRootProps>(
  function PlaybackChromeRoot({ onRetryOutline, canEnterProMode, onEnterProMode }, ref) {
    ...
    // MASAR: session start timestamp + fired guard for the session-end hook.
    const masarSessionStartRef = useRef<number>(Date.now());
    const masarSessionEndFiredRef = useRef(false);
    ...
    // MASAR Tasks 9+11: fire the session-end capture → Analyzer → Session Log
    // exactly once per sitting. Fire-and-forget: never blocks or breaks the UI.
    const fireMasarSessionEnd = useCallback(() => {
      if (masarSessionEndFiredRef.current) return;
      masarSessionEndFiredRef.current = true;
      void import('@/lib/masar/session-end').then(async (m) => {
        const payload = await m.captureAndLogSessionEnd(
          m.sessionEndContextFromStore(masarSessionStartRef.current),
        );
        if (payload) await m.analyzeAndLogSession(payload);
      });
    }, []);
    ...
    useEffect(() => {
      ...
      const initializeScene = async () => {
        ...
        const engine = new PlaybackEngine([currentScene], actionEngine, audioPlayerRef.current, {
          ...
          // Lazy narration TTS: the speech line has no cached clip; resolve it
          // from the scene's background prefetch. The engine races this
          // against a timeout of its own, so resolve fast with null when
          // nothing is pending rather than waiting for generation ourselves.
          ensureSpeechAudio: (action) => {
            const pending = awaitSceneSpeech(currentScene.id);
            if (!pending) return Promise.resolve(null);
            return pending.then(() => {
              // The prefetcher mutated (and possibly split) the actions array;
              // re-read from the store to see the stamped audioId — the
              // engine's action object may be a stale reference.
              const scene = useStageStore
                .getState()
                .scenes.find((s) => s.id === currentScene.id);
              const updated = scene?.actions?.find(
                (a) => a.type === 'speech' && a.id === action.id,
              ) as SpeechAction | undefined;
              return updated?.audioId ?? null;
            });
          },
          ...
          onComplete: () => {
            ...
            // MASAR Task 9: reached the end of the LAST scene → completed session.
            const stageState = useStageStore.getState();
            const allScenes = stageState.scenes;
            if (allScenes[allScenes.length - 1]?.id === currentScene.id) {
              fireMasarSessionEnd();
            }
        ...
        // Lazy narration TTS: scenes ship without audio; start baking this
        // scene's clips the moment it activates so the first spoken line
        // (and every later one) finds them ready or nearly ready. Fire-and-
        // forget — playback never waits on it (the engine's bounded
        // ensureSpeechAudio wait is the only rendezvous).
        if (sceneNeedsSpeechAudio(currentScene)) {
          prefetchSceneSpeech(currentScene, stage?.languageDirective);
        }
    ...
    useEffect(() => {
      ...
      return () => {
        ...
        // MASAR Task 9: student left without finishing → partial/abandoned session.
        // Fires only if the completed-path hook hasn't already fired.
        fireMasarSessionEnd();
