      ...
      case 'speech': {
        ...
        this.audioPlayer
          ...
          .then(async (audioStarted) => {
            ...
            // Lazy TTS: narration is generated at scene activation, so the
            // scene's first line may reach the engine while its clip is still
            // baking. Wait a bounded beat for it — a voice-over beats the
            // silent reading dwell for the line the user is watching — then
            // fall through to the normal fallbacks. The background prefetch
            // keeps running either way, so later lines play from cache.
            ...
            if (managedTTS && this.callbacks.ensureSpeechAudio) {
              try {
                const assetId = await Promise.race([
                  this.callbacks.ensureSpeechAudio(speechAction),
                  new Promise<null>((resolve) => setTimeout(() => resolve(null), 8_000)),
                ]);
                // Scene switch / stop / pause happened while waiting: a
                // superseded engine must not start audio now.
                if (!this.isCurrentGeneration(generation)) return;
                if (assetId) {
                  speechAction.audioId = assetId;
                  const started = await this.audioPlayer.play(assetId);
                  if (!this.isCurrentGeneration(generation)) return;
                  if (started) return; // onEnded chain advances playback
                }
              } catch (err) {
                if (!this.isCurrentGeneration(generation)) return;
                log.warn('lazy speech audio wait failed:', err);
              }
            }

            // No pre-generated audio (and none arrived in time) — try
            // browser-native TTS only when it is the selected provider AND
            // actually enabled (opt-in, #665).
            if (
              settings.ttsEnabled &&
              settings.ttsProviderId === 'browser-native-tts' &&
              isTTSProviderEnabled(
