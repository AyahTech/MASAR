...
/**
 * Simple global async lock. Used below to serialize Kimi free-tier calls,
 * which enforce max organization concurrency = 1. Acquiring the lock waits
 * until the previous holder releases it; releasing is done by calling the
 * returned function. Never throws during acquire/release.
 */
class AsyncLock {
...
/** One global lock for all Kimi (Moonshot) HTTP requests. */
const kimiFetchLock = new AsyncLock();

function isKimiRequest(url: string): boolean {
  return url.includes('moonshot');
}
...
function isStreamingChatRequest(init?: RequestInit): boolean {
  if (!init?.body || typeof init.body !== 'string') return false;
  try {
    return (JSON.parse(init.body) as Record<string, unknown>).stream === true;
  } catch {
    return false;
  }
}

async function fetchWithKimiLock(
  input: RequestInfo | URL,
  init?: RequestInit,
): Promise<Response> {
  const url = requestUrlString(input);
  if (!isKimiRequest(url)) {
    return globalThis.fetch(input, init);
  }

  const release = await kimiFetchLock.acquire();
  const response = await globalThis.fetch(input, init);

  // For streaming requests we must hold the lock until the response body is
  // fully consumed. Kimi counts an open streaming connection as "in flight",
  // so releasing immediately after fetch() returns would let the next caller
  // hit the concurrency limit. Pump the stream through a pass-through
  // TransformStream and release only when it closes.
  if (!isStreamingChatRequest(init) || !response.body) {
    await delay(1_500);
    release();
    return response;
  }
  ...
  void (async () => {
    try {
      ...
      // Cooldown so Kimi's server-side concurrency counter decrements before
      // the next caller acquires the lock.
      await delay(1_500);
      release();
    }
  })();
...
async function fetchCustomOpenAIChat(
  ...
  if (!requestUrl.includes('/chat/completions') || !init?.body || typeof init.body !== 'string') {
    return fetchWithKimiLock(input, init);
  }
  ...
  if (requestBody.stream === true) return fetchWithKimiLock(input, init);
  ...
  const response = await fetchWithKimiLock(input, {
    ...init,
    body: JSON.stringify({
      ...requestBody,
      stream: true,
    ...
    case 'openai': {
      ...
      if (config.providerId !== 'openai') {
        ...
        const compatFetch = async (url: RequestInfo | URL, init?: RequestInit) => {
          ...
          const response = await fetchWithKimiLock(url, init);
