...
export const TTS_PROVIDERS: Record<BuiltInTTSProviderId, TTSProviderConfig> = {
  ...
  'elevenlabs-tts': {
    ...
    // All three models are multilingual (Arabic included). eleven_flash_v2 is
    // deliberately NOT offered: it is English-only and would silently produce
    // garbled Arabic. Multilingual voices = model handles the language; the
    // voice sets the timbre.
    // Offline fallback set — the user's own voice library (incl. Arabic voices)
    // is fetched from the account via /api/elevenlabs-voices and stored in
    // ttsProvidersConfig['elevenlabs-tts'].customVoices.
    voices: [
      {
