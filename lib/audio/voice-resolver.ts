 ...
 * For built-in providers with user-fetched account voices (e.g. ElevenLabs),
 * those are merged on top of the static catalog by id (account entry wins).
 */
export function getServerVoiceList(
  ...
  if (!provider) return [];
  const accountVoices = getBuiltInAccountVoices(providerId, ttsProvidersConfig);
  if (accountVoices.length === 0) return provider.voices.map((v) => v.id);
  const byId = new Map(provider.voices.map((v) => [v.id, v.id]));
  for (const voice of accountVoices) byId.set(voice.id, voice.id);
  return [...byId.values()];
}

/**
 * User-fetched account voices stored on a BUILT-IN provider's config
 * (`ttsProvidersConfig[providerId].customVoices` — e.g. voices pulled from the
 * user's ElevenLabs voice library). Persisted like any provider setting; the
 * static catalog in TTS_PROVIDERS is untouched.
 */
function getBuiltInAccountVoices(
  providerId: TTSProviderId,
  ttsProvidersConfig?: Record<string, unknown>,
): Array<{ id: string; name: string; language?: string }> {
  if (!ttsProvidersConfig || !Object.hasOwn(TTS_PROVIDERS, providerId)) return [];
  const entry = ttsProvidersConfig[providerId] as
    | { customVoices?: Array<{ id: string; name: string; language?: string }> }
    | undefined;
  return entry?.customVoices || [];
}
...
export function getEnabledProvidersWithVoices(
  ...
  for (const [id, config] of Object.entries(TTS_PROVIDERS)) {
    ...
    {
      // Account-fetched voices (e.g. the user's ElevenLabs voice library).
      // Merged by id over the static catalog — an account entry wins so the
      // picker shows the user's own name/language for premade ids too.
      const accountVoices = getBuiltInAccountVoices(providerId, ttsProvidersConfig);
      const accountById = new Map(accountVoices.map((v) => [v.id, v]));
      const catalogVoices = config.voices.map((v) => ({
        id: v.id,
        name: accountById.get(v.id)?.name || v.name,
        language: accountById.get(v.id)?.language || v.language,
      }));
      const accountOnlyVoices = accountVoices
        .filter((v) => !config.voices.some((catalog) => catalog.id === v.id))
        .map((v) => ({ id: v.id, name: v.name, language: v.language || '' }));

      const allVoices = [...catalogVoices, ...accountOnlyVoices, ...userVoices];
      ...
      if (config.models.length > 0) {
        for (const model of config.models) {
          ...
          // Account voices carry no compatibleModels metadata; they are usable
          // with every model of their provider (verified for ElevenLabs).
          compatibleVoices.push(...accountOnlyVoices);
...
export function findVoiceDisplayName(
  ...
  // Account-fetched voices (e.g. ElevenLabs library) override the static
  // catalog's display name by id.
  const accountVoice = getBuiltInAccountVoices(providerId, ttsProvidersConfig).find(
    (v) => v.id === voiceId,
  );
  if (accountVoice) return accountVoice.name;
