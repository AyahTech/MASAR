...
export interface SettingsState {
  ...
  ttsProvidersConfig: Record<
    TTSProviderId,
    {
      ...
      /** Custom provider voices, or account-fetched voices on built-in providers (e.g. ElevenLabs library). */
  ...
  persist(
      ...
      return {
        ...
        setTTSProviderConfig: (providerId, config) =>
          set((state) => {
            ...
            // Saving a credential for a provider lights TTS up: enable it
            // globally and point the selection at that provider (same first-run
            // behavior server-configured providers get in fetchServerProviders).
            // The deterministic fallback then routes every agent's voice through
            // it. Fires only on the unconfigured → configured transition, and an
            // explicit `enabled: false` in the same write opts out.
            const nextConfig = ttsProvidersConfig[providerId as keyof typeof ttsProvidersConfig];
            const transitionedToConfigured =
              !state.ttsEnabled &&
              config.enabled !== false &&
              isTTSProviderConfigured(providerId, nextConfig) &&
              !isTTSProviderConfigured(
                providerId,
                state.ttsProvidersConfig[providerId as keyof typeof ttsProvidersConfig],
              );
            if (transitionedToConfigured) {
              return {
                ttsProvidersConfig,
                ttsEnabled: true,
                ttsProviderId: providerId,
    ...
    },
    {
      ...
      merge: (persistedState, currentState) => {
        ...
        // Demo safety reset: cloud TTS providers (ElevenLabs, etc.) hit rate
        // limits on free tiers and stall generation. Force browser-native TTS
        // on every rehydrate so the UI cannot accidentally use a failing provider.
        if (typedMerged.ttsProviderId && typedMerged.ttsProviderId !== 'browser-native-tts') {
          typedMerged.ttsProviderId = 'browser-native-tts';
        }
