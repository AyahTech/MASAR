...
export async function generateAndStoreTTS(
  ...
  const assetId =
    replaceAssetId ??
    (await withStorageTimeout('Asset pool write', putAsset(blob, assetMeta)));
  if (replaceAssetId)
    await withStorageTimeout('Asset pool replace', replaceAsset(replaceAssetId, blob, assetMeta));
...
/**
 * Storage-write watchdog. IndexedDB writes on a disk-pressured machine (or
 * with a second tab holding the DB open) can stall indefinitely WITHOUT ever
 * erroring — a hung `put` blocks the whole TTS clip queue and the generation
 * UI stays on "Generating Teaching Actions" forever. Racing the write against
 * a timeout turns the stall into a normal error so the retry wrapper (and,
 * failing that, the non-blocking TTS path) can recover instead of hanging.
 */
const STORAGE_WRITE_TIMEOUT_MS = 30_000;

function withStorageTimeout<T>(label: string, work: Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(
        new Error(
          `${label} timed out after ${STORAGE_WRITE_TIMEOUT_MS / 1000}s (storage stalled — check disk space / other tabs)`,
        ),
      );
 ...
 *
 * NOT called by the generation pipeline anymore: narration is generated
 * lazily at classroom playback (lib/audio/playback-tts-prefetch.ts) so a
 * slow/rate-limited TTS provider can never stretch the "actions" phase or
 * fail a scene. Callers: the playback prefetcher and per-line edit-mode
 * regeneration paths.
 */
export async function generateTTSForScene(
  ...
  // its own audio under its own audioId, with no cross-action ordering — so
  // render them with bounded concurrency instead of one at a time. The floor is
  // 3 (a live-demo-friendly default that turns N×5s of serial clips into
  // ~N/3×5s); the server's PARALLEL_SCENE_CONCURRENCY raises it for capable
  // providers, and an explicit -1 restores the original strictly-serial
  // behaviour (for keys with per-key concurrency quotas of 1).
  const serverConcurrency = Math.floor(
    useSettingsStore.getState().parallelSceneConcurrency ?? 0,
  );
  const ttsConcurrency =
    serverConcurrency < 0 ? 0 : Math.max(3, serverConcurrency);
  try {
...
export function useSceneGenerator(options: UseSceneGeneratorOptions = {}) {
  ...
  const generateRemaining = useCallback(
    async (params: GenerationParams) => {
      ...
      // Pipelined generation loop (#572). When parallelism is on, each scene's
      // *content* fetch is kicked off up front with bounded concurrency
      // (lazyBoundedMap) and its *actions* fetch is chained behind that scene's
      // own content — but results are CONSUMED IN ORDER inside the serial loop
      // below; there is no barrier. The first scene paints after content(1) +
      // actions(1) (same as serial) while later scenes' fetches run hidden
      // behind earlier scenes' assembly. Content has no cross-scene dependency;
      // actions depend only on their OWN scene's content, so per-scene chaining
      // is safe. The previousSpeeches transition cue is snapshotted at actions
      // kick-off (latest known): for pipelined scenes it may lag, and the
      // prompt formatter omits the cue gracefully — global coherence comes
      // from allOutlines, present in every prompt. TTS is not part of
      // generation at all (lazy at classroom playback). With parallelism off
      ...
      try {
        ...
        // Actions pre-warm, chained behind each scene's OWN content promise.
        // A scene's actions need only that scene's content, so the chain can
        // start the moment content resolves — overlapping the next scene's
        // content fetch instead of waiting for the serial loop to reach it.
        // previousSpeeches is read WHEN CONTENT RESOLVES (fresher than a
        // registration-time snapshot); the value may still trail the ideal by
        // a scene or two, which only softens the optional transition cue.
        // Never rejects.
        const actionsPromises = useParallelContent
          ? new Map<string, Promise<SceneActionsResult>>()
          : null;
        if (actionsPromises && contentPromises) {
          for (const outline of pending) {
            const contentPromise = contentPromises.get(outline.id);
            if (!contentPromise) continue;
            actionsPromises.set(
              outline.id,
              contentPromise.then((contentResult): Promise<SceneActionsResult> | SceneActionsResult => {
                // Undefined/skipped (abort/epoch change) or failed content:
                // propagate the miss; the serial loop handles the failed-
                // content branch for this outline before consuming us.
              ...
              }).catch((err): SceneActionsResult => ({
                success: false,
                error: err instanceof Error ? err.message : 'Actions generation failed',
              })),
            );
          }
        }
        ...
        for (const outline of pending) {
          ...
          // Step 2: Generate actions + assemble scene. Parallel mode: await
          // this scene's pre-warmed chain (content already resolved; actions
          // usually well underway or done). Serial mode: fetch now.
          const warmedActions = actionsPromises?.get(outline.id);
          const actionsResult = await (warmedActions ??
            fetchActions(
              contentResult.effectiveOutline || outline,
              contentResult.content,
              previousSpeeches,
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            // No TTS here: narration clips are generated lazily when a scene is
            // activated in the classroom (lib/audio/playback-tts-prefetch.ts).
            // Generating them inline made the "actions" phase crawl behind a
            // rate-limited TTS API and let one bad clip fail the whole scene.
  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      ...
      try {
        ...
        // No TTS here either — lazy narration at classroom playback (see the
        // comment in generateRemaining).
