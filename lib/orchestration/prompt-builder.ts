...
/**
 * MASAR Task 13b: cultural perspective section — extends (never replaces) the
 * archetype persona. Rendered into the agent system prompt only when the agent
 * carries a culturalPersona; empty string otherwise (template slot renders
 * nothing, upstream behavior preserved).
 */
export function buildCulturalPersonaSection(country: string): string {
  return `\n# Cultural Perspective
ROLE: as defined by your personality above.
PERSPECTIVE: You are a university student from ${country}.

Bring authentic viewpoints, examples, and framings from this background when
they genuinely add to the discussion. Vary your examples across the session.
Never reduce your background to a stereotype, and never speak as though all
people from ${country} think alike. You are one individual student with your
own opinions, not a national representative.\n`;
}
...
export function buildStructuredPrompt(
  ...
  const vars = {
    ...
    culturalPersonaSection: agentConfig.culturalPersona
      ? buildCulturalPersonaSection(agentConfig.culturalPersona)
      : '',
