...
import { adaptationToTeacherContext, runPathAgent } from '@/lib/masar/path-agent';
import {
...
export interface GenerateClassroomInput {
  ...
  /** MASAR: learner identity for the Path Agent (session-log adaptation). */
  learnerKey?: string;
}
...
export async function generateClassroom(
  ...
  // MASAR Task 12: Path Agent — adapt this session to the student's history
  // before outlines are generated. Failures fall back to default parameters
  // inside runPathAgent; generation must never break because of it.
  // courseId: the generation is per-requirement, so the course partition is the
  // requirement's stable digest — the same requirement text maps to the same
  // course across sessions.
  let masarTeacherContext = '';
  if (input.learnerKey) {
    try {
      const courseKey = requirement.trim().slice(0, 120);
      const adaptation = await runPathAgent(input.learnerKey, courseKey);
      masarTeacherContext = adaptationToTeacherContext(adaptation);
      log.info(
        `PathAgent: ${adaptation.isDefault ? 'default' : adaptation.difficultyAdjustment}${masarTeacherContext ? ' (teacherContext injected)' : ''}`,
      );
    } catch (e) {
      log.warn('PathAgent failed; continuing with default parameters:', e);
    }
  }
  ...
  const outlinesResult = await generateSceneOutlinesFromRequirements(
    ...
    aiCall,
    {
      ...
      researchContext,
      // MASAR: Path Agent adaptive guidance injected into the outline prompt.
      teacherContext: masarTeacherContext || undefined,
    },
  );
