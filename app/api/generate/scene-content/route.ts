...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const aiCall = async (
      ...
      // Some free/slow providers (Kimi, DeepSeek) can take minutes to generate
      // a full slide. Cap the single LLM call below the route's 5-minute
      // serverless budget so a hung call does not exhaust the whole request.
      const abortController = new AbortController();
      const timeoutId = setTimeout(() => abortController.abort(), 240_000);
      ...
      try {
        if (images?.length && hasVision) {
          ...
          // bytes the base64 path sends BEFORE prompt assembly, keeping the
