...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // MASAR Task 12: the Path Agent's adaptive guidance joins the teacher
    // context — same prompt slot, appended below the agent personas.
    let teacherContext = formatTeacherPersonaForPrompt(agents);
    const learnerKeyHeader = req.headers.get('x-learner-key');
    if (learnerKeyHeader) {
      try {
        const adaptation = await runPathAgent(
          learnerKeyHeader,
          requirements.requirement.trim().slice(0, 120),
        );
        const guidance = adaptationToTeacherContext(adaptation);
        if (guidance) {
          teacherContext = `${teacherContext}\n${guidance}`;
          log.info(
            `PathAgent: ${adaptation.isDefault ? 'default' : adaptation.difficultyAdjustment} (guidance injected)`,
          );
        }
      } catch (e) {
