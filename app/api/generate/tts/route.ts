...
/**
 * Upstream watchdog: race the provider call against a hard deadline so a
 * stalled upstream (rare, but it happens — e.g. ElevenLabs holding a
 * connection open) surfaces as a 504-style error the client's retry wrapper
 * can act on, instead of a clip request that hangs until the platform kills
 * the route. Generous enough for slow providers (VoxCPM local synthesis);
 * retries make the rare false-positive cheap.
 */
const UPSTREAM_TTS_TIMEOUT_MS = 25_000;

function withUpstreamTimeout<T>(work: Promise<T>): Promise<T> {
  return new Promise<T>((resolve, reject) => {
    const timer = setTimeout(() => {
      reject(new Error(`TTS upstream timed out after ${UPSTREAM_TTS_TIMEOUT_MS / 1000}s`));
    }, UPSTREAM_TTS_TIMEOUT_MS);
    work.then(
      (value) => {
        clearTimeout(timer);
        resolve(value);
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Generate audio (watchdogged — see withUpstreamTimeout)
    const { audio, format } = await withUpstreamTimeout(generateTTS(config, text));
