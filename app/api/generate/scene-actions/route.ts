...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const {
      ...
      modelInfo,
      modelString,
      thinkingConfig: clientThinkingConfig,
    } = await resolveModelFromRequest(req, body, 'scene-actions');

    // Actions are a short JSON-array task; reasoning adds significant latency
    // (especially on DeepSeek) with no quality benefit for this stage. Default
    // to disabled thinking unless the client explicitly enabled it.
    const thinkingConfig: ThinkingConfig | undefined = clientThinkingConfig?.enabled
